...
#  mutate(LAB = replace(LAB, LAB == '>2000', floor(runif(1, min=2000, max=2500)))) -> lab_data
...
m4 <- glm(data$Death ~  1 + data$logodds + lab_data$LAB, family = binomial)
m4

data$m4_pred <- predict(m4, type = "response")

x <- cal_plot2(m4, "original", "m4_pred", model_label ='' )
plot(x)
```

```{r}
rocobj <- plot.roc(data$Death, data$m4_pred,
                   main = "Confidence intervals", 
                   percent=TRUE,
                   ci = TRUE,                  # compute AUC (of AUC by default)
                   print.auc = TRUE)           # print the AUC (will contain the CI)
