...
```{r}
src_data_file <- "edp-data-ctn.csv"
...
msfkail_data <- df_edp[df_edp$SiteID == "MSF_Kail", ]
msfmag_data <- df_edp[df_edp$SiteID == "MSF_Mag", ]
...
Removal of the high missingness variables in ELWA data results in accepting MCAR. Now, examining CTE-NZEREKORE, MSF-Kail, MSF-Mag, 1-Bongo, 3-Margibi, and 4-Kambia visually because those cause the MCAR test to crash
...
```{r}
print("MSF_Kail")
test_data <- msfkail_data[unlist(test_vars)]
vis_miss(test_data)
```

```{r}
print("MSF_Mag")
test_data <- msfmag_data[unlist(test_vars)]
vis_miss(test_data)
```
...
In all these cases the results of the test failing to run is because of entire columns missing, or no missing data. Removal of those would result in MCAR.
...
The saturated model has a AUC (C statistic) on the training data of 0.780, and an optimism in Dxy of 0.0652. Since C and Dxy are related by C = 0.5 + Dxy/2, the optimism corrected C is ~0.754, so the saturated model has a degree of overfitting to the data, although is not too bad. Some clinical symptoms have ORs lower than 1. This seems to indicate they increase the odds of survival, which does not make medical sense.
